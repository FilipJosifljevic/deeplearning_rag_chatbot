...
recursive_character_splitter = RecursiveCharacterTextSplitter(chunk_size=500, chunk_overlap=100)
recursive_character_splitter_bigger = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=200)
...
def get_recursively_split_chunks(docs):
    return recursive_character_splitter.create_documents(["\n".join(docs)])

def get_recursively_split_chunks_bigger(docs):
    return recursive_character_splitter_bigger.create_documents(["\n".join(docs)])
